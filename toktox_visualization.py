...
def _draw_scalars_movie(axes, tt, times, t_now, flux_con_tm, flux_con_tx):
    ...
    # 2: V_loop and flux consumed
    ax = axes[1]
    ax.plot(times, s['vloop_tm'], color=COLOR_TM, ls=LS_PRI, lw=LW, marker=MK_TM, ms=MK_SZ, label='Vloop TM')
    ax.plot(times, s['vloop_tx'], color=COLOR_TX, ls=LS_PRI, lw=LW, label='Vloop TX')
    ax.set_ylabel('V_loop [V]', fontsize=LABEL_FS)
    ax.legend(fontsize=LEGEND_FS, loc='upper left')
    _style(ax)
    ax2 = ax.twinx()
    ax2.plot(times, flux_con_tm, color=COLOR_TM, ls=LS_SEC, lw=LW, marker=MK_TM, ms=MK_SZ, label='\u03a6 TM')
    ax2.plot(times, flux_con_tx, color=COLOR_TX, ls=LS_SEC, lw=LW, label='\u03a6 TX')
    ...
    # 3: pax
    ax = axes[2]
    ax.plot(times, np.array(s['pax_tm']) / 1e3, color=COLOR_TM, ls=LS_PRI, lw=LW, marker=MK_TM, ms=MK_SZ, label='pax TM')
    ax.plot(times, np.array(s['pax']) / 1e3, color=COLOR_TX, ls=LS_PRI, lw=LW, label='pax TX')
    ax.set_ylabel('pax [kPa]', fontsize=LABEL_FS)
    ax.legend(fontsize=LEGEND_FS, loc='upper left')
    _style(ax)

    # 4: Power channels and Q
    ax = axes[3]
    pkeys = [
    ...
    if coil_data:
        ...
        for ci, (cname, cvals) in enumerate(sorted(coil_data.items())):
            ...
            ax.plot(ct, ci_vals, ls=LS_PRI, lw=LW * 0.7, color=coil_colors[ci], label=cname)
